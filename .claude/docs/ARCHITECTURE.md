...
- **Authentication**: every screen wraps its content in `<LoginGate>`. The bearer token is
  ...
  reads — it is not a component and cannot call `useAuth()`. Both of the gate's waiting
  branches — session being resolved, session missing — render the same branded welcome
  block, not plain text ([#170](https://github.com/zelytra/Librarius/issues/170)).
- **Waiting**: `<Loading>` (`shared/ui/states.tsx`) is the only thing in the app that draws
  a wait, in two formats — `large` for a screen with nothing to show, `compact` for an
  action in flight — and it owns the two thresholds that keep a fast answer from flashing
  anything (`LOADING_DELAY_MS`, `LOADING_MIN_VISIBLE_MS`). A screen mounts it and unmounts
  it; an action leaves it mounted and passes its mutation's `isPending`, which is what lets
  the minimum visible duration outlive the end of the wait. Its animated mark is a
  placeholder for the logo animation planned later: replacing it is that component plus one
  CSS rule ([#169](https://github.com/zelytra/Librarius/issues/169)).
